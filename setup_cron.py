...
def dashboards(name, command):
  ...
  return f"python3 ~/home-dashboard/dashboards/{name}/{command}.py >> ~/home-dashboard/dashboards/{name}/{command}.log 2>&1"

def renderers(name):
  if name not in DASHBOARDS:
    raise ValueError('Invalid dashboard name!')
  return f"python3 ~/home-dashboard/renderers/render_{name}.py >> ~/home-dashboard/dashboards/{name}/render.log 2>&1"
